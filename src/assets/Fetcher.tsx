...
const Fetcher = () => {
    ...
    const [selectedProducts, setSelectedProducts] = useState<Table[] | null>(null);
    ...
    useEffect(()=>{
        const FetchAPI = async (pageNumber: number)=>{
            ...
            const noOfRowsSelected : number = (selectedRows ?? 0) - ( rowsPerPage * (page - 1));
            if(noOfRowsSelected > 0){
                const data = [...json.data.slice(0,noOfRowsSelected)];
                const ids = localStorage.getItem(('ids'));
                const array = ids?.split(',');
                const filteredArray = data.filter(item=> !array?.includes(item.id.toString()));
        ...
        return (
            <div >
                ...
                <OverlayPanel ref={op}>
                    <input 
                        type="text" 
                        placeholder="Submit rows.."
                        ref={inputRef}/>
                    <button 
                        onClick={()=>{
                            setSelectedRows(Number(inputRef.current?.value))
                            if(inputRef.current?.value)
                                localStorage.setItem('selectedRows',inputRef.current?.value.toString())}}
                    >   Submit
                    </button>
                </OverlayPanel>
            </div>
  ...
  return (
    <div>
      ...
      >     
            ...
            <Column field="date_start" header="Year start" style={{ width: '25%' }}></Column>
            <Column field="date_end" header="Year End" style={{ width: '25%' }}></Column>
      </DataTable>
